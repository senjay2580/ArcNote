...
| 2.6 正式存储 & 元数据入库 | - 文件本体：移至对象存储（S3/MinIO）或加密文件服务器，存储路径示例：`/storage/file/202512/8f9c7d8e.docx`；<br>- 文件名脱敏：存储名替换为随机串，原始名存入数据库；<br>- 元数据入库：文件ID、用户ID、存储路径、MD5、上传时间、权限标识 | - 加密存储：敏感文件用AES-256加密，密钥存KMS（如阿里云KMS）；<br><br>                                                                         |
...
| 步骤         | 操作细节                                                                                                               | 技术实现/避坑点                                                                             |
| ---------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| 1.1 发起导出请求 | - 携带导出参数（如数据范围、文件格式、脱敏规则）；<br>- 携带认证信息（JWT Token）；<br>- 提示用户“导出可能需要时间，完成后将通知”                                      | - 请求接口：`POST /api/v1/file/export`（POST避免参数过长）；                                       |
