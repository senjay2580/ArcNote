...
```ad-note
SSO（单点登录）的实现方式，是围绕「**身份凭证的生成、传递、验证**」这一核心逻辑

# SSO实现方式对比表
| 实现方式                | 核心原理                                                                 | 优点                                                                 | 缺点                                                                 |
|-------------------------|--------------------------------------------------------------------------|----------------------------------------------------------------------|----------------------------------------------------------------------|
| 方式1：URL参数传递      | 直接将Token拼接在URL参数中，在系统间传递                                 | ✅ 简单直接<br/>✅ 兼容性好<br/>✅ 不需要额外配置                      | ❌ Token暴露在URL中（安全性较低）<br/>❌ 浏览器历史记录会保存Token     |
| 方式2：Cookie + 顶级域名 | 把Token存入顶级域名（如.example.com）的Cookie中，子域名系统共享该Cookie   | ✅ Token不暴露在URL中<br/>✅ 浏览器自动携带Cookie，无需手动处理        | ❌ 要求所有系统归属同一顶级域名<br/>❌ 跨域场景下配置和调试复杂度高    |
| 方式3：iframe + postMessage | 借助iframe嵌入目标系统页面，通过postMessage API跨域传递Token             | ✅ Token不暴露在URL中<br/>✅ 支持不同域名的跨域SSO场景                | ❌ 实现逻辑复杂，需处理iframe通信、鉴权等细节<br/>❌ 每个子系统需开发专属的Token接收页面 |

### 补充说明（可选）
1. **方式1（URL参数）**：适合对安全性要求低、系统架构简单的内部场景，是本项目当前使用方式；
2. **方式2（Cookie+顶级域名）**：适合同顶级域名下的多子系统场景，需注意Cookie的过期时间、Secure/HttpOnly等安全配置；
3. **方式3（iframe+postMessage）**：适合跨顶级域名的复杂SSO场景，需严格校验postMessage的消息来源（origin），避免XSS攻击。
```
