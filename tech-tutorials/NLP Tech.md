...
|   |── theme-green            # 放置主题相关文件，用于切换主题（命名:theme-主题色）
...
组件-elementUI https://element.eleme.cn/2.0/#/zh-CN/component/

1、使用size:small，没有size的部分，以产品经理为主

2、组件的size使用配置，详情查看src/theme-setting.js

3、对话框无需全屏，dialog弹窗 按钮放在右下角

4、遮罩为全部遮罩

5、表格操作采用文字说明，不使用图标

6、表格右上方操作按钮（统一要求图标加文字的方式）要右对齐表格，并且和表格之间的间距为8px
...
搜索区域参考 https://panjiachen.github.io/vue-element-admin/#/table/complex-table

1、查询字段前面不需要说明，用placeholder代替

2、中间间隔10px

3、查询按钮直接接在表单后面，新增等操作按钮另起一行
...
1.所有.js,.scss文件 命名采用单个单词小写，多个单词以'-'连接 eg:permission.js,common-config.js

2.除了index.vue 其他.vue文件 命名采用大驼峰 eg:index.vue,App.vue,AppMain.vue

3.全局使用的组件放置components/大驼峰组件命名/index.vue

  单个路由页面使用的组件放置以error-page为例 eg:views/error-page/components/大驼峰组件命名.vue

4.所有文件按照文件夹结构对应放置，全局使用的文件直接放置文件夹下，模块使用的文件放置在modules文件夹（没有则新建）下，

  与路由页面文件同名,多级路由同理

  以api文件夹为例，新建对应请求文件 api方法命名建议与后台接口方法命名一致
...
以新增test-page为例,命名均对应菜单名称

1.在views新建test-page文件夹，多级路由则新建test-page-nested文件夹,结构如下

  有页面级别组件 src/views/test-page/components/大驼峰组件命名.vue，src/views/test-page/index.vue(页面文件)

  无页面级别组件 src/views/test-page/index.vue(页面文件)

  注意：页面文件第一行应注明菜单路径，以及页面说明！！！ eg:// 企业中心->企业资料->基本信息

2.新增路由，src/router/modules/test-page.js

3.页面请求文件，src/api/modules/test-page.js

4.模拟数据文件，mock/modules/test-page.js
...
1.开发过程中注意页面换肤，尽量不要在页面写死有关主题的样式，如果需要，则需要在对应主题文件中提供覆盖入口

2.开发过程中发起的请求相关参数直接放在api路径下

3.前端开发完成之后，自己在mock文件夹下先做数据模拟调用，调用方式见src/utils/request.js注释，联调和提交测试时，改为正式请求并提交git，尽量做到前端不需要依赖后台进度

4.非变动以及新增变量的情况，禁止提交所有本地开发配置文件！！！

5.页面风格统一，统一在页面布局最外层加上<div class="app-container"></div>,app-container是统一做页面边距控制的类

6.页面样式尽量使用scoped，避免影响到其他页面

7.提交代码时，属于不同模块的文件分开提交，分开填写提交说明，方便在后续追溯

8.开发过程中遇到非业务类方法（eg:日期格式化等），请提取出来，放置在src\utils，并根据功能命名（eg:src\utils\formatDate.js）
