...
-------------------------------------------------------------------
```ad-help
加密算法是信息安全的核心，根据**加密逻辑**（对称/非对称/哈希）和**应用场景**可分为三大类，以下是主流算法、特性及典型应用场景的详细梳理：

### 一、核心分类：对称加密、非对称加密、哈希算法

### 三类加密 / 哈希算法核心差异对比表

|类型|核心特征|典型优势|典型劣势|
|---|---|---|---|
|对称加密|加密与解密使用**同一把密钥**（如 AES、DES）|运算速度极快、资源消耗低，适合大批量数据加密|密钥分发 / 传输易泄露，多节点场景下密钥管理成本高|
|非对称加密|公钥加密、私钥解密（或私钥签名、公钥验签），密钥对不互通（如 RSA、ECC）|密钥无需传输，管理更安全，支持身份认证 / 数字签名|运算速度慢、资源消耗大，不适合大文件加密|
|哈希算法|不可逆（无解密逻辑）、定长输出、雪崩效应（如 MD5、SHA-256）|可校验数据完整性（防篡改），运算效率较高|无法还原原文，存在哈希碰撞风险（需选强哈希算法）|
...
| 算法       | 核心参数                | 安全性 | 性能 | 适用场景                          |
|------------|-------------------------|--------|------|-----------------------------------|
| AES（高级加密标准） | 密钥长度：128/192/256位；分组长度128位 | 极高  | 极快 | 绝大部分场景的首选（替代DES）|
| DES/3DES   | DES：56位密钥（已破解）；3DES：3次DES加密 | 低（DES）/中（3DES） | 中   | 老旧系统兼容（逐步淘汰）|
| SM4（国密） | 128位密钥，分组加密     | 高     | 快   | 国内金融、政务、物联网            |
| ChaCha20   | 流加密，无需分组        | 高     | 快（无硬件加速时优于AES） | 移动端/物联网（无AES硬件加速场景） |

#### 2. 典型应用场景
- **AES**：
  - 本地数据加密：手机/电脑硬盘加密（BitLocker）、APP本地缓存敏感数据（如用户手机号）；
  - 传输加密：HTTPS的TLS握手后的数据传输（对称加密为主，非对称仅用于密钥协商）、VPN数据加密；
  - 存储加密：数据库敏感字段（如身份证、银行卡号）、云存储文件加密（如阿里云OSS加密）。
- **SM4**：
  - 国内金融系统：银行卡交易数据加密、网银APP数据传输；
  - 物联网：智能家居设备间通信加密、工业物联网数据传输。
- **ChaCha20**：
  - 移动端：微信/支付宝的弱网环境数据传输、Android/iOS系统的轻量级加密；
  - 区块链：部分公链的交易数据加密（如比特币的轻节点通信）。

### 三、第二类：非对称加密算法（公钥加密）
#### 1. 主流算法及特性
| 算法       | 核心原理                | 安全性 | 性能 | 适用场景                          |
|------------|-------------------------|--------|------|-----------------------------------|
| RSA        | 大整数因式分解（密钥长度1024/2048/4096位） | 高（2048位以上） | 慢   | 通用场景（密钥协商、数字签名）|
| ECC（椭圆曲线加密） | 椭圆曲线离散对数问题（密钥长度256位≈RSA 3072位） | 极高  | 较快（比RSA省资源） | 移动端/物联网/区块链              |
| SM2（国密） | 椭圆曲线算法（国产）| 高     | 快   | 国内政务、金融、电子签名          |
| DSA/ECDSA  | 数字签名专用（基于离散对数/椭圆曲线） | 高     | 中   | 数字签名（如代码签名、证书验签）|

#### 2. 典型应用场景
- **RSA**：
  - 密钥协商：HTTPS握手时，客户端用服务器公钥加密「对称密钥」，传输给服务器（避免对称密钥明文传输）；
  - 数字签名：软件安装包签名（如Windows软件的数字签名）、邮件加密（PGP/GPG）；
  - 身份认证：SSH登录（免密登录的公钥认证）、VPN的身份验证。
- **ECC/ECDSA**：
  - 区块链：比特币/以太坊的钱包地址生成、交易签名（256位ECC密钥即可保证安全，且体积小）；
  - 移动端：5G通信的身份认证、手机SIM卡的加密鉴权；
  - 物联网：智能手表/传感器的轻量级身份验证（节省硬件资源）。
- **SM2**：
  - 国内电子签名：电子合同（如法大大、e签宝）、政务系统的身份认证；
  - 金融：央行数字货币（数字人民币）的交易签名、银行U盾的加密算法。

### 四、第三类：哈希算法（散列函数，不可逆）
#### 1. 主流算法及特性
| 算法       | 输出长度                | 安全性 | 性能 | 适用场景                          |
|------------|-------------------------|--------|------|-----------------------------------|
| MD5        | 128位                   | 低（已破解，碰撞易产生） | 极快 | 仅用于非安全场景（文件校验）|
| SHA-1      | 160位                   | 低（已破解） | 快   | 老旧系统兼容（逐步淘汰）|
| SHA-256/SHA-512 | 256/512位             | 极高  | 快   | 安全场景首选                      |
| SM3（国密） | 256位                   | 高     | 快   | 国内政务、金融                    |
| HMAC（哈希消息认证码） | 基于MD5/SHA-256，带密钥 | 高     | 快   | 接口验签、防篡改                  |

#### 2. 典型应用场景
- **SHA-256**：
  - 数据完整性校验：下载文件的哈希值验证（如Linux镜像的SHA256校验）、区块链区块哈希（比特币区块哈希用SHA-256）；
  - 密码存储：用户密码加盐哈希存储（如MySQL的password()函数、Spring Security的密码加密）；
  - 数字签名：配合非对称加密（如RSA+SHA256），对签名内容做哈希后再加密（减少计算量）。
- **MD5**：
  - 非安全场景：文件重复检测（如网盘秒传）、日志内容摘要（仅用于标识，不用于验真）；
  - 注意：严禁用于密码存储、数据防篡改（易被碰撞攻击）。
- **HMAC-SHA256**：
  - 接口验签：API接口请求参数加签（如微信支付/支付宝接口，避免参数被篡改）；
  - 消息认证：即时通讯消息的完整性验证（如微信消息防篡改）。
- **SM3**：
  - 国内金融：银行卡交易数据的哈希校验、社保数据的完整性验证；
  - 政务：电子证照的哈希摘要（用于验真）。

### 五、扩展：混合加密（主流实际应用模式）
单一算法无法满足所有需求，实际场景中多为「非对称+对称+哈希」混合使用：
#### 示例1：HTTPS加密流程
1. 非对称加密（RSA/ECC）：客户端用服务器公钥加密「随机对称密钥」，传输给服务器（解决对称密钥分发问题）；
2. 对称加密（AES）：后续所有数据传输用对称密钥加密（保证速度）；
3. 哈希算法（SHA-256）：对传输数据做哈希，验证完整性（防篡改）。

#### 示例2：用户登录流程
1. 客户端：对用户密码做「加盐SHA-256哈希」后传输（避免密码明文）；
2. 服务端：用HMAC-SHA256验证哈希值（防参数篡改）；
3. 服务端存储：密码哈希值（不可逆，即使数据库泄露也无法还原密码）。

### 六、算法选型总结
| 场景需求                | 推荐算法组合                          |
|-------------------------|---------------------------------------|
| 本地/传输数据加密（快+安全） | AES-256 + 随机密钥                    |
| 密钥分发/身份认证        | RSA-2048/ECC-256 + AES-256            |
| 密码存储                | SHA-256 + 随机盐 + 多次哈希（如PBKDF2） |
| 接口验签/数据防篡改      | HMAC-SHA256                           |
| 国内政务/金融            | SM2（非对称）+ SM4（对称）+ SM3（哈希） |
| 区块链/移动端            | ECC-256 + AES-128 + SHA-256           |

### 七、安全注意事项
1. 避免使用老旧算法：DES、MD5、SHA-1已淘汰，优先选AES-256、RSA-2048+、SHA-256；
2. 密钥管理：对称密钥需安全存储（如硬件加密模块HSM），非对称私钥严禁泄露；
3. 加盐处理：哈希密码时必须加「随机盐」（避免彩虹表攻击）；
4. 国密合规：国内金融、政务系统需优先使用SM2/SM3/SM4（符合《商用密码管理条例》）。
```
#### 1.1 密码学基本概念
...
安全校验前置、传输加密可靠、存储分离规范、操作可追溯

## 一、文件导入流程（docx/xlsx/pdf等）

### 整体链路

前端预校验 → 分片/直传 → 后端全量校验 → 安全扫描 → 解析处理 → 存储归档 → 结果反馈 → 日志审计

#### 1. 前端核心操作（减无效请求）

- **基础过滤**：用accept限定文件白名单，校验后缀与大小（≤100MB），仅做体验优化（不可信）。
    
- **元信息处理**：spark-md5算文件MD5（防重传+验完整），生成唯一上传ID（用户ID+时间戳+随机串）。
    
- **上传策略**：≤5MB直传，>5MB分片上传（5MB/片），支持断点续传；HTTPS传输，Token放Authorization头。
    
- **结果处理**：接收文件ID/错误码（如1001=类型非法），展示用户可理解的提示（隐藏服务器敏感信息）。
    

#### 2. 后端核心操作（防风险）

- **鉴权限流**：拦截器校验Token与上传权限，Redis限流（单用户1小时≤10次上传），校验Referer防CSRF。
    
- **文件校验**：魔数（如docx=504B0304）+结构双重校验，拒绝改后缀/畸形文件。
    
- **安全扫描**：ClamAV/火绒查杀病毒，OCR+正则检测敏感信息，清除Office文件宏。
    
- **解析存储**：沙箱环境解析文件（防漏洞），文件存S3/MinIO（AES-256加密），元数据（路径/MD5/权限）存数据库。
    
- **日志归档**：记录用户ID/IP/文件ID/操作结果，日志加密不可篡改。
    

#### 3. 避坑重点

文件不存数据库；大文件必分片+断点续传；解析用沙箱；24小时清理临时文件。

## 二、文件导出流程（xlsx/csv/pdf等）

### 整体链路

前端请求 → 后端权限校验 → 数据脱敏 → 文件生成 → 加密存储 → 临时链接 → 前端下载 → 日志审计

#### 1. 前端核心操作（合规下载）

- **发起请求**：POST传导出参数（数据范围/格式/脱敏规则），携带JWT Token。
    
- **状态处理**：小数据量直取链接，大数据量轮询任务状态（3-5秒间隔），给出进度提示。
    
- **安全下载**：通过临时链接（5分钟有效期+HMAC签名）下载，依赖浏览器原生能力。
    

#### 2. 后端核心操作（控数据）

- **权限校验**：RBAC校验导出权限，JSR303/Pydantic校验参数，限流（1小时≤3次导出）。
    
- **数据处理**：单次导出≤10万条，脱敏敏感信息（手机号/身份证等），符合《个人信息保护法》。
    
- **文件生成**：用成熟库流式生成（Java=EasyExcel，Python=pandas），添加导出人+时间水印。
    
- **下载管控**：生成带签名的临时链接，后端转发文件流（隐藏存储URL），设置正确响应头（Content-Disposition等）。
    
- **日志清理**：记录导出范围/下载时间，7天自动清理过期文件，日志保留≥6个月。
    

#### 3. 避坑重点

大数据量异步+流式生成；必脱敏；临时链接需签名+过期；禁止暴露存储公网URL。

## 三、通用规范

1. **传输安全**：全链路HTTPS，敏感文件可额外前端加密。
    
2. **存储规范**：文件存对象存储，元数据存数据库，密钥分离存KMS。
    
3. **监控告警**：监控高频操作/超大文件/异地访问，触发阈值即告警。
    
4. **合规要求**：公示存储政策并获授权，记录数据出境情况，满足等保2.0。
```