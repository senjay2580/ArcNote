...
已读未读的逻辑一般如何实现的 可以先从 Hula中学习技术方案 和自己的技术方案对比（画流程图 假设自己要开发这个接口！）
IM 已读未读 机制
1. 事件合并（核心性能提升）
短时间内（如 1 秒）同一用户 + 同一聊天的多次 “标记已读” 事件（如用户快速滚动消息），合并为 1 次，仅用最大的maxMsgId更新数据库；
例：用户滚动到 msgId=100，又滚动到 msgId=200，合并后仅处理 msgId=200 的事件，避免 2 次数据库更新。
2. 幂等性保障
事件 ID+Redis 标记：消费前检查事件是否已处理，避免 MQ 重试导致重复更新；
数据库层面：ON DUPLICATE KEY（未读计数）、status != 3（仅更新未读消息），双重保障幂等。
3. 缓存先行（前端体验）
接口返回前先更新 Redis 缓存，前端获取 “未读计数 / 已读状态” 时优先读缓存，保证实时性；
数据库异步更新完成后，缓存与数据库最终一致。
4. 批量消费 + 批量更新
MQ 消费者批量消费（如每批 10 条事件），数据库批量更新，减少数据库 IO 次数（1 次批量更新≈10 次单条更新的性能）；
数据库 SQL 加FOR UPDATE SKIP LOCKED：避免行锁等待，提升并发更新效率。
5. 失败重试
MQ 自带重试机制（如失败后重试 3 次，间隔 5 秒），避免单次异常导致数据不一致；
重试仍失败时，可接入告警系统（如钉钉 / 短信），人工介入处理。
