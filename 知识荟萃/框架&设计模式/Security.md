...
| 维度     | 传统 Session/Cookie 方式                | JWT 方式                                |
| ------ | ----------------------------------- | ------------------------------------- |
| 凭证核心形态 | 「无意义的随机标识」（Session ID）              | 「有意义的自包含令牌」（JWT 字符串）                  |
| 凭证存储位置 | 服务端存储（Session 数据存在服务端内存 / Redis）    | 客户端存储（Token 存在前端 LocalStorage/Cookie） |
| 验证逻辑   | 中心化验证：服务端通过 Session ID 查库 / 缓存取用户信息 | 去中心化验证：服务端仅验签，无需查库即可解析用户信息            |
| 状态依赖   | 有状态：服务端需维护 Session 的生命周期、存储状态       | 无状态：服务端不存储任何用户认证状态                    |
