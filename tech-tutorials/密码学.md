[第一章 密码学](#%E7%AC%AC%E4%B8%80%E7%AB%A0-%E5%AF%86%E7%A0%81%E5%AD%A6)
-------------------------------------------------------------------
...
密码学是网络安全、[信息安全](https://so.csdn.net/so/search?q=%E4%BF%A1%E6%81%AF%E5%AE%89%E5%85%A8&spm=1001.2101.3001.7020)、区块链等产品的基础，常见的非对称加密、对称加密、[散列函数](https://so.csdn.net/so/search?q=%E6%95%A3%E5%88%97%E5%87%BD%E6%95%B0&spm=1001.2101.3001.7020)等，都属于密码学范畴。
...
公元前 5 世纪的时候，斯巴达人利用一根木棒，缠绕上皮革或者羊皮纸，在上面横向写下信息，解下这条皮带。展开来看，这长串字母没有任何的意义。
...
这难不倒解密者，以英文字母为例，为了确定每个英文字母的**出现频率**，分析一篇或者数篇普通的英文文章，英文字母出现频率最高的是 e，接下来是 t，然后是 a……，然后检查要破解的密文，也将每个字母出现的频率整理出来，假设密文中出现频率最高的字母是 j，那么就可能是 e 的替身，如果密码文中出现频率次高的但是 P，那么可能是 t 的替身，以此类推便就能解开加密信息的内容。这就是频率分析法。
...
我们修改 编码格式 , 编码格式改成 GBK ，我们在运行发现变成2个字节
...
[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-9M4IgYta-1608474746593)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img\DES 报错. png)]

修改 密钥 `key = “12345678”` ，再次运行 ，出现乱码是因为对应的字节出现负数，但负数，没有出现在 ascii 码表里面，所以出现乱码，需要配合 base64 进行转码

[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-xlpqo2ll-1608474746596)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img\DES 输出. png)]

使用 `base64` 进行编码

`base64` 导包的时候，需要注意 ，别导错了，需要导入 `apache` 包

[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-wcsGcXGu-1608474746601)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img\DES 修改. png)]
...
base64 是 3 个字节为一组，一个字节 8 位，一共 就是 24 位 ，然后，把 3 个字节转成 4 组，每组 6 位，3×8=4×6

3 * 8 = 4 * 6 = 24 ，每组 6 位（然后扩展为 8 位 1B），缺少的 2 位，会在高位进行补 0 ，这样做的好处在于 ，base 取的是后面 6 位，去掉高 2 位 ，那么 base64 的取值就可以控制在 0-63 位了，所以就叫 base64，111 111 = 32 + 16 + 8 + 4 + 2 + 1 =
...
注意 key 是 16 个字节即可，DES 的 key 是 8 字节
...
[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-svD16lxv-1608474746605)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img\AES 报错. png)]
...
前面 DES 和 AES 都有 key，而加密模式是说怎么用 key。

ECB 是同 key 并行，CBC 是变 key 串行

##### 1.7.1 [ECB](#ecb)

ECB : Electronic codebook, 电子密码本. 需要加密的消息按照块密码的块大小被分为数个块，并对每个块进行独立加密

但是每个块使用的是同一个 key，所以可以并行处理

[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-GJp3FHOl-1608474746606)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img\ECB.png)]
...
在加密的时候，会取决于前面的 iv 相邻，把前面的相邻进行异或处理，后面的铭文进行加密的时候，会一直依赖于前面的加密 key

[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-ZPoSAC1W-1608474746609)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img\CBC.png)]

*   优点 : 同样的原文生成的密文不一样
*   缺点 : 串行处理数据

注意 CBC 传参的时候不只要 key，还要有一个`IV向量`
...
另外在 CBC 中还要在加密解密中都写好 IV 向量`IvParameterSpec iv = new IvParameterSpec(key.getBytes());`

IV 这个字节数组可以自己定，但必须是 8 个字节
...
*   但要注意 DES 和 AES 的限制，不能不俗 8/16 字节的整数倍
*   iv 字节也必须是 8 个字节
...
[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-ATghX2zQ-1608474746610)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img \ 填充错误修改 1.png)]
...
[外链图片转存失败, 源站可能有防盗链机制, 建议将图片保存下来直接上传 (img-bkYwvrJw-1608474746611)(C:\Users\Administrator\iCloudDrive \ 笔记 \ java 系列 \ 安全密码学 \ img \ 填充错误修改 2.png)]

在测试 `AES` 的时候需要注意，key 需要 16 个字节，加密向量也需要 16 个字节 ，其他方式跟 `DES` 一样
...
> 比如有的网站下载软件有 sha256 等值，可以下载该软件，然后得到其消息摘要，比对官网的消息摘要，就知道该软件有没有被修改过
...
数字摘要转换成 16 进制：虽然我们不能显示负数，但是我们可以显示字符串，把原来的字节码不用 ASCII 表示，改用 16 进制表示即可
...
> 要注意的是，要有补 0 的操作，原因在于 Integer.toHexString(b & 0xff); 这个语句，如果字节是 00001111 的话，只得到了 F
...
生成 RSA 的公钥私钥：`KeyPairGenerator`类
...
数字签名（又称[公钥](https://baike.baidu.com/item/%E5%85%AC%E9%92%A5)数字签名）是只有信息的发送者才能产生的别人无法伪造的一段数字串，这段数字串同时也是对信息的发送者发送信息真实性的一个有效证明。它是一种类似写在纸上的普通的物理签名，但是使用了[公钥加密](https://baike.baidu.com/item/%E5%85%AC%E9%92%A5%E5%8A%A0%E5%AF%86/6245950)领域的技术来实现的，用于鉴别数字信息的方法。一套数字签名通常定义两种互补的运算，一个用于签名，另一个用于验证。
...
![](https://i-blog.csdnimg.cn/blog_migrate/9571e5bddfbdc0333b37cfcbdd8821ad.png)

服务器把内容（证书明文）经过 hash 算法得到证书的摘要，再经过私钥加密得到证书的数字签名，然后发送证书和数字签名给浏览器。

浏览器得到了证书和数字签名，先拿证书里的公钥对对数字签名解密得到摘要。再拿证书里的 hash 算法对证书进行 hash 得到摘要。对比连个摘要是否相同。如果相同，则代表证书没有被修改过，也就证明了是该服务器发过来的。

但是如果别人伪造了一个证书呢？比如证书里写的颁发给了 baidu，而且公钥是第三者的，你发送的内容就被别人拦截下来用第三者的私钥解密，偷看后为了不被发现再用第三者的私钥加密发送给 baidu。
...
> 服务器有公钥 A，私钥 A。认证机构有私钥 B，公钥 B
> 
> 服务器把公钥 A 给认证机构，为了让认证中心给自己开个证明说我这个公钥确实是我的，而认证中心自己系统里也知道我给谁鉴定过以及他的公钥。
> 
...
我们看一个应用 “数字证书” 的实例：https 协议。这个协议主要用于网页加密
...
运行中输入 certlm.msc ，打开证书管理器

#### 1.12 keytool 工具使用

keytool 工具路径：`C:\Program Files\Java\jre1.8.0_91\bin`
...
(1) 生成密钥证书 下边命令生成密钥证书，采用 RSA 算法每个证书包含公钥和私钥
...
配置 openssl 的 path 环境变量，如下图：

本教程配置在 C:\OpenSSL-Win64\bin

cmd 进入 guigu.jks 文件所在目录执行如下命令 (如下命令在 windows 下执行，会把 - 变成中文方式，请将它改成英文的 -)：
...
将上边的公钥拷贝到文本 public.key 文件中，合并为一行, 可以将它放到需要实现授权认证的工程中。

[第二章 SpringBoot 集成 Swagger2](#%E7%AC%AC%E4%BA%8C%E7%AB%A0-springboot%E9%9B%86%E6%88%90swagger2)
-----------------------------------------------------------------------------------------------
...
但是项目开发中的沟通成本也随之升高，这部分沟通成本主要在于前端开发人员与后端开发人员对 WebAPI 接口的沟通，Swagger2 就可以很好地解决，它可以动态生成 Api 接口文档，降低沟通成本，促进项目高效开发。

有时候定义了文档，代码中修改了一点小的东西，总会忘记同步修改文档，时间长了，自己都比较蒙，还需要看一下代码才能发现问题。

##### 2.2.0 [swagger](https://so.csdn.net/so/search?q=swagger&spm=1001.2101.3001.7020) 介绍

OpenAPI 规范（OpenAPI Specification 简称 OAS）是 Linux 基金会的一个项目，试图通过定义一种用来描述 API 格  
式或 API 定义的语言，来规范 RESTful 服务开发过程，目前版本是 V3.0，并且已经发布并开源在 github 上。  
`（https://github.com/OAI/OpenAPI-Specification）`  
Swagger 是全球最大的 OpenAPI 规范（OAS）API 开发工具框架，支持从设计和文档到测试和部署的整个 API 生命周  
期的开发。`(https://swagger.io/)`  
Spring Boot 可以集成 Swagger，生成 Swagger 接口，Spring Boot 是 Java 领域的神器，它是 Spring 项目下快速构建  
项目的框架

##### 2.2.1 swagger 的基础注解介绍

swagger 通过注解生成接口文档，包括接口名、请求方法、参数、返回信息的等等。
...
2.1、@Api 修饰整个类，描述 Controller 的作用
...
参数和 @ApiImplicitParam 一致，只是这个注解可以添加多个参数而已
...
点击 try it out 输入姓名， Execute 执行，返回如下图效果
...
如果在请求服务器的时候，程序被人篡改，把价格改成 6 元，运行就会购物失败


# 第三章 文件导入导出的安全问题

文件导入导出的最佳实践核心是**“安全校验前置、传输加密可靠、存储分离规范、操作可追溯”**，以下是分场景（导入/导出）的前后端完整流程，包含关键安全节点、技术实现细节和避坑点，适配企业级平台通用场景。

## 一、文件导入流程（以上传docx/xlsx/pdf为例）
### 整体流程
前端预校验 → 分片/直传 → 后端多重校验 → 安全扫描 → 解析处理 → 存储归档 → 结果反馈 → 日志审计

#### 1. 前端侧流程（核心：减少无效请求，提升体验）
| 步骤 | 操作细节 | 技术实现/避坑点 |
|------|----------|----------------|
| 1.1 选择文件 & 基础校验 | - 仅展示业务允许的文件类型（白名单），隐藏其他类型；<br>- 校验文件后缀+预估大小（如≤100MB）；<br>- 提示用户“禁止上传含宏/敏感信息的文件” | - 白名单示例：`accept=".docx,.xlsx,.pdf,.jpg"`（仅前端过滤，不可信）；<br>- 文件大小：通过`file.size`判断，超出则禁用上传按钮并提示；<br>- 避坑：不要依赖前端校验（可被绕过），仅做体验优化。 |
| 1.2 生成文件元信息 | - 计算文件MD5/SHA256（防止重复上传+校验完整性）；<br>- 生成唯一上传ID（如用户ID+时间戳+随机串）；<br>- 记录原始文件名、文件类型、上传者信息 | - 推荐库：`spark-md5`（前端计算大文件MD5，分片计算避免卡顿）；<br>- 上传ID格式示例：`upload_10086_20251210_8f9c7d8e`。 |
| 1.3 分片/直传选择 | - 小文件（如≤5MB）：直接上传；<br>- 大文件（>5MB）：分片上传（如5MB/片），支持断点续传 | - 分片实现：`File.slice(start, end)`切割文件，按片编号上传；<br>- 断点续传：前端记录已上传分片编号，上传前先请求后端“查询已上传分片”，跳过已传部分；<br>- 传输协议：强制HTTPS，请求头携带`Content-Type: application/octet-stream`。 |
| 1.4 上传请求 & 进度反馈 | - 携带认证信息（JWT Token/Token放在请求头，禁止URL传参）；<br>- 上传接口：`POST /api/v1/file/upload`（或分片接口`/api/v1/file/upload/chunk`）；<br>- 实时展示上传进度（`XMLHttpRequest.upload.onprogress`） | - 避坑：Token放在`Authorization: Bearer xxx`头，防止泄露；<br>- 超时处理：设置超时时间（如30s），超时后自动重试（最多3次）。 |
| 1.5 接收上传结果 | - 后端返回“上传成功/失败+文件ID/错误信息”；<br>- 失败时提示具体原因（如“文件含病毒”“格式非法”）；<br>- 成功后跳转/刷新文件列表 | - 错误码标准化：如1001=文件类型非法，1002=文件过大，1003=病毒检测失败；<br>- 避坑：不展示敏感错误信息（如“服务器路径不存在”），仅返回用户可理解的提示。 |

#### 2. 后端侧流程（核心：全维度校验，防范安全风险）
| 步骤               | 操作细节                                                                                                                                                | 技术实现/避坑点                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1 接口鉴权 & 限流    | - 校验Token有效性、用户是否有上传权限；<br>- 限流：单用户/IP 1小时内最多上传10次，单文件最多重试5次；<br>- 校验请求来源（Referer/Origin），防范CSRF                                                    | - 鉴权：统一拦截器校验Token，无权限直接返回403；<br>- 限流：使用Redis做计数器（如`incr upload_limit_10086`，过期时间1小时）；<br>- 避坑：允许合法跨域的Origin，避免拦截正常请求。                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 2.2 接收文件 & 临时存储  | - 分片上传：接收分片后暂存到临时目录（如`/tmp/upload/chunk/`），合并前校验分片完整性；<br>- 直传：接收文件流，暂存到临时目录（非业务存储目录）                                                               | - 临时目录权限：仅后端进程可读写，禁止777权限；<br>- 合并分片：按分片编号排序，通过`FileChannel`合并，合并后删除临时分片。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2.3 多重格式校验       | - 魔数校验（真实文件类型）：如docx魔数`504B0304`、pdf魔数`25504446`；<br>- 结构校验：对Office/PDF文件解析头部结构，拒绝畸形文件；<br>- 最终过滤：仅保留白名单内的文件类型                                      | - 魔数校验工具：Java用`Files.probeContentType()`，Python用`filemagic`；<br>- 避坑：禁止仅通过后缀名判断文件类型，必须魔数+结构双重校验。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2.4 安全扫描 & 内容检测  | - 病毒查杀：调用ClamAV/火绒接口扫描文件，高危文件直接删除；<br>- 敏感内容检测：OCR识别图片/PDF，正则匹配身份证/银行卡等敏感信息；<br>- 宏清除：对Office文件强制清除宏代码（如用Apache POI删除宏）                             | - 扫描异步处理：大文件可先返回“待审核”，扫描通过后再标记为“可用”；<br>- 隔离机制：可疑文件移至隔离目录，禁止解析/访问。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2.5 文件解析（可选）     | - 如需提取文件内容（如导入Excel数据）：在沙箱环境（Docker/专用解析服务）解析；<br>- 解析后校验数据格式（如Excel列数、字段类型）；<br>- 解析失败则返回“数据格式错误”                                                  | - 沙箱隔离：解析服务与主服务隔离，防止解析漏洞攻击主服务；<br>- 解析库：Java用Apache POI，Python用openpyxl，及时更新版本修复漏洞。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2.6 正式存储 & 元数据入库 | - 文件本体：移至对象存储（S3/MinIO）或加密文件服务器，存储路径示例：`/storage/file/202512/8f9c7d8e.docx`；<br>- 文件名脱敏：存储名替换为随机串，原始名存入数据库；<br>- 元数据入库：文件ID、用户ID、存储路径、MD5、上传时间、权限标识 | - 加密存储：敏感文件用AES-256加密，密钥存KMS（如阿里云KMS）；<br>- 数据库表设计：<br>```sql<br>CREATE TABLE file_meta (<br>  id BIGINT PRIMARY KEY AUTO_INCREMENT,<br>  file_uid VARCHAR(64) UNIQUE COMMENT '文件唯一标识',<br>  original_name VARCHAR(255) COMMENT '原始文件名',<br>  storage_path VARCHAR(512) COMMENT '存储路径/URL',<br>  file_md5 VARCHAR(64) COMMENT '文件MD5',<br>  file_size BIGINT COMMENT '文件大小(字节)',<br>  user_id BIGINT COMMENT '上传者ID',<br>  file_type VARCHAR(32) COMMENT '文件类型',<br>  status TINYINT COMMENT '0:待审核 1:可用 2:隔离',<br>  create_time DATETIME DEFAULT CURRENT_TIMESTAMP<br>);<br>``` |
| 2.7 日志审计 & 结果返回  | - 记录上传日志：用户ID、IP、文件ID、操作时间、结果；<br>- 返回文件ID、存储状态、解析结果（如有）；<br>- 日志加密存储，不可篡改                                                                          | - 日志字段：`log_id, user_id, ip, file_uid, operation_type, result, create_time`；<br>- 避坑：日志不记录文件内容/MD5以外的敏感信息。                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |

### 导入流程避坑重点
1. 禁止将文件直接存入数据库（仅元数据入库）；
2. 大文件必须分片+断点续传，避免超时/中断；
3. 解析文件必须在沙箱环境，防止解析漏洞（如XXE、RCE）；
4. 临时文件及时清理（如定时任务删除24小时内未处理的临时文件）。

## 二、文件导出流程（以导出xlsx/csv/pdf/docx为例）
### 整体流程
前端请求 → 后端权限校验 → 数据查询&脱敏 → 文件生成 → 加密存储 → 生成临时下载链接 → 前端下载 → 日志审计

#### 1. 前端侧流程（核心：合规请求，安全下载）
| 步骤 | 操作细节 | 技术实现/避坑点 |
|------|----------|----------------|
| 1.1 发起导出请求 | - 携带导出参数（如数据范围、文件格式、脱敏规则）；<br>- 携带认证信息（JWT Token）；<br>- 提示用户“导出可能需要时间，完成后将通知” | - 请求接口：`POST /api/v1/file/export`（POST避免参数过长）；<br>- 参数示例：<br>```json<br>{<br>  "data_range": {"start_time": "2025-12-01", "end_time": "2025-12-10"},<br>  "file_format": "xlsx",<br>  "desensitize": true // 是否脱敏<br>}<br>``` |
| 1.2 处理导出响应 | - 同步导出（小数据量）：直接返回下载链接；<br>- 异步导出（大数据量）：返回导出任务ID，前端轮询`/api/v1/file/export/status/{taskId}`查询状态；<br>- 导出完成后展示“下载按钮” | - 轮询间隔：3-5秒，避免高频请求；<br>- 避坑：异步导出需给用户反馈（如“导出中，预计5分钟完成”）。 |
| 1.3 触发文件下载 | - 点击下载按钮，请求临时下载链接（`GET /api/v1/file/download/{fileUid}`）；<br>- 下载链接携带签名，且有效期短（如5分钟）；<br>- 浏览器原生下载（避免自定义下载逻辑导致安全风险） | - 临时链接生成：后端用HMAC签名（如`签名=HMAC-SHA256(文件ID+时间戳+密钥)`）；<br>- 避坑：禁止直接暴露文件存储路径，必须通过后端转发下载。 |
| 1.4 下载完成处理 | - 提示“下载成功”，记录下载时间；<br>- （可选）自动清理前端临时缓存的导出参数 | - 下载失败：提示“链接已过期/权限不足”，引导重新生成下载链接。 |

#### 2. 后端侧流程（核心：数据合规，下载可控）
| 步骤 | 操作细节 | 技术实现/避坑点 |
|------|----------|----------------|
| 2.1 权限 & 参数校验 | - 校验用户是否有导出权限（如仅管理员可导出全量数据）；<br>- 校验导出参数合法性（如时间范围不超过90天）；<br>- 限流：单用户1小时内最多导出3次，避免大数据量导出压垮数据库 | - 权限校验：基于RBAC，校验用户角色是否包含`EXPORT_FILE`权限；<br>- 参数校验：用JSR303（Java）/Pydantic（Python）校验参数格式。 |
| 2.2 数据查询 & 脱敏 | - 按参数查询数据，限制查询条数（如单次导出≤10万条）；<br>- 敏感数据脱敏：手机号隐藏中间4位、身份证隐藏后6位、地址隐藏详细门牌号；<br>- 数据格式校验：确保字段类型匹配导出格式（如数字字段不导出为文本） | - 脱敏工具：Java用Hutool的`DesensitizedUtil`，Python用`pydantic-extra-types`；<br>- 避坑：脱敏规则需符合合规要求（如《个人信息保护法》），且可配置。 |
| 2.3 生成导出文件 | - 选择成熟的生成库，避免手写文件格式（易出错）；<br>- 大数据量：流式生成文件（避免内存溢出）；<br>- 文件添加水印（如“导出人：XXX 导出时间：XXX”） | - 生成库选型：<br>  - Excel：Java用EasyExcel（流式生成），Python用pandas+openpyxl；<br>  - PDF：Java用iText7，Python用ReportLab；<br>  - Word：Java用POI，Python用python-docx；<br>- 水印实现：Excel/PDF/Word均支持添加文字水印，防止文件泄露后溯源。 |
| 2.4 文件存储 & 生成临时链接 | - 生成的文件存入对象存储/文件服务器（同导入存储规则）；<br>- 生成临时下载链接：包含文件UID、签名、过期时间；<br>- 链接示例：`/api/v1/file/download?fileUid=xxx&sign=xxx&expire=1736400000` | - 签名校验：后端接收到下载请求时，校验签名是否有效、是否过期；<br>- 避坑：临时链接仅允许单次下载或有限次数下载（如最多3次）。 |
| 2.5 下载请求处理 | - 校验下载链接的签名、过期时间、用户权限；<br>- （可选）校验请求来源（Referer/Origin）；<br>- 后端转发文件流（禁止直接返回存储URL），设置响应头：<br>  - `Content-Disposition: attachment; filename="导出数据_20251210.xlsx"`（指定下载文件名）；<br>  - `Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`（正确的MIME类型）；<br>  - `Content-Length: 文件大小` | - 大文件下载：流式返回文件流（Java用`ResponseEntity<Resource>`，Python用`StreamingResponse`），避免内存溢出；<br>- 避坑：文件名编码（如中文转UTF-8），防止乱码。 |
| 2.6 日志审计 & 清理 | - 记录导出日志：用户ID、IP、文件ID、导出数据范围、下载时间；<br>- 定时清理导出文件（如保留7天，到期自动删除）；<br>- 日志保留≥6个月，满足合规要求 | - 清理策略：通过定时任务（如XXL-Job）删除过期文件，同时更新数据库文件状态为“已删除”；<br>- 避坑：删除文件时需同步删除元数据或标记状态，避免死链接。 |

### 导出流程避坑重点
1. 大数据量导出必须异步+流式生成，禁止一次性加载所有数据到内存；
2. 导出文件必须脱敏，尤其是包含个人信息/企业机密的内容；
3. 临时下载链接必须签名+过期，防止链接被滥用；
4. 禁止直接返回对象存储的公网URL，必须通过后端鉴权后转发。

## 三、通用最佳实践（导入+导出）
### 1. 传输安全
- 全链路HTTPS，禁止HTTP传输；
- 敏感文件（如含隐私数据）可在传输层额外加密（如前端加密后上传，后端解密存储）。

### 2. 存储规范
- 文件本体：对象存储（S3/MinIO）为主，文件服务器为辅，禁止存数据库；
- 元数据：数据库存储（MySQL/PostgreSQL），包含权限、状态、存储路径等；
- 加密：敏感文件AES-256加密存储，密钥与文件分离（存KMS）。

### 3. 监控与告警
- 监控异常操作：高频上传/下载、超大文件导出、异地IP导出敏感文件；
- 告警阈值：单用户1小时上传>10个文件、单次导出>10万条数据、文件扫描命中病毒特征。

### 4. 合规性
- 公示文件存储政策（存储位置、留存时间、加密方式），获取用户授权；
- 导出数据需记录“数据出境”（如有），符合跨境数据传输规则；
- 日志不可篡改，满足等保2.0/三级等保要求。
