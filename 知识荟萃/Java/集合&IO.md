...
并行流是基于 Fork/Join 框架实现的**多线程流式处理**，将流的元素拆分到多个线程并行处理，提升大数据量下的处理效率。

- 串行流：单线程处理，`stream()`；
- 并行流：多线程处理，`parallelStream()` 或 `stream().parallel()`。
**Fork/Join 框架将任务拆分为多个子任务（Fork），子任务并行执行，最终合并结果（Join），默认使用`ForkJoinPool.commonPool()`（核心数 = CPU 核心数 - 1）。**
...
## 函数式接口
| 接口类型  | 内置接口            | 抽象方法              | 用途             | 场景示例          |
| ----- | --------------- | ----------------- | -------------- | ------------- |
| 无参无返回 | `Runnable`      | `void run()`      | 无输入无输出         | 线程任务、异步执行     |
| 有参无返回 | `Consumer<T>`   | `void accept(T)`  | 消费一个参数         | 遍历集合打印元素      |
| 有参有返回 | `Function<T,R>` | `R apply(T)`      | 转换 / 处理参数并返回结果 | 字符串转长度、对象属性提取 |
| 布尔判断  | `Predicate<T>`  | `boolean test(T)` | 条件过滤           | 集合筛选、参数校验     |
| 无参有返回 | `Supplier<T>`   | `T get()`         | 生产数据           | 生成随机数、懒加载对象   |
