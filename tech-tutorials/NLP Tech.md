...
```ad-warning
|维度|数据库游标（Database Cursor）|游标分页（Cursor Pagination）|
|---|---|---|
|**本质定义**|数据库端逐行处理结果集的机制（数据处理工具）|应用层的分页方案（数据查询工具）|
|**核心目的**|逐行操作数据（读 / 改），解决批量操作无法覆盖的单行逻辑|高效分页加载大数据量列表，解决 OFFSET 分页性能问题|
|**操作对象**|数据库内部的结果集|应用层对前端暴露的分页数据|
|**依赖条件**|数据库连接、结果集|唯一且有序的字段（如 msg_id）+ 索引|
|**性能特点**|逐行处理，效率低，占用数据库连接|范围查询，效率极高，无性能衰减|
|**适用场景**|离线 / 低并发的数据处理（清理、归档）|高并发、大数据量的分页查询（加载历史消息）|
...
- **核心逻辑**：不搞 “一刀切”，根据数据量（1 万条为阈值）拆分「同步 / 异步」两条链路，适配不同场景的性能和体验需求；
- **落地体现**：小数据量直接同步返回文件流（低延迟、低成本），大数据量走异步队列（避免超时、释放 Web 线程）；
- **底层思路**：用 “场景分层” 解决单一方案无法兼顾 “响应速度” 和 “大吞吐量” 的矛盾。

#### 2. 「异步解耦」的非阻塞设计思维

- **核心逻辑**：将 “请求接收” 和 “数据处理” 彻底解耦，避免长任务占用核心 Web 线程池，突破请求超时限制；
- **落地体现**：Controller 仅生成任务 ID、存储状态、投递消息，实际导出逻辑由消息队列消费线程处理；
- **底层思路**：用 “异步化” 把同步阻塞的 “长链路” 拆成多个短链路，每个链路只做单一职责，提升系统容错性和吞吐量。

#### 3. 「流式控资」的内存安全思维

- **核心逻辑**：拒绝 “全量加载数据到内存”，通过 “分段、流式、缓冲” 三重手段严格控制内存占用，从根源避免 OOM；
- **落地体现**：
    - 数据查询：用游标分页 / MyBatis 流式查询，逐批读取而非全量加载；
    - 数据写入：EasyExcel 禁用内存模式，超出阈值自动刷盘到磁盘；
    - 数据缓冲：阻塞队列限制缓存批数（10 批），避免生产者过快导致队列堆积占满内存；
- **底层思路**：用 “流式处理” 将 “一次性加载” 转为 “逐批处理”，让内存占用始终可控在预设阈值内。

#### 4. 「全链路可观测」的稳定性保障思维

- **核心逻辑**：给长耗时异步任务加 “状态追踪”，让每一步操作可监控、可回溯、可重试；
- **落地体现**：Redis 存储任务全生命周期状态（待处理 / 处理中 / 完成 / 失败），实时更新处理进度，前端轮询感知状态，失败时记录错误信息支持重试；
- **底层思路**：用 “可观测性” 解决异步任务 “黑盒化” 问题，把 “不可控的长任务” 转为 “可追踪、可干预的可控任务”。
```
...
思想：
1. **中心化管控思维：拒绝 “点对点信任”，只信任统一的 “中心节点”**
    
    所有子系统不直接互相传递敏感数据（Token），而是只和「IdP 统一认证中心」交互：
    
    - 子系统 A 只告诉 IdP “要跳转到 B”，不碰 Token；
    - IdP 作为唯一的可信中心，负责验证用户身份、保管 Token、安全推送给目标子系统；
    - 本质是把 “子系统之间的信任”，简化为 “所有子系统只信任 IdP”，避免了多系统互信的安全风险和复杂度。
---
